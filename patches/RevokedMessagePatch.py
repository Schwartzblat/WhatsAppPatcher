...
class RevokedMessagePatch(Patch):
    ...
    REVOKE_MESSAGE_RE = re.compile('\.method (?:\w+ )*\w+\(LX\/\w+;Z\)V\s*\.\w+ \w+\s*(.*?)\.end method', re.DOTALL)
    ...
    def __init__(self, extracted_path):
        ...
        self.print_message = (
            "[+] Patching revoke message method..."
        )
    ...
    def class_modifier(self, class_data) -> str:
        ...
        return class_data.replace(revoke_function_data, self.REVOKE_MESSAGE_REPLACE+revoke_function_data)
